...
class DancersModel extends Db
{
    ...
    function update()
    {
        $sql = "UPDATE `sgostu_dancer` SET dancer_firstname = ?, dancer_lastname = ?, dancer_firstname_en = ?, dancer_lastname_en = ?, dancer_birth = ?, dancer_sex = ?, dancer_index = ?, dancer_street = ?, dancer_house = ?, dancer_flat = ?, dancer_phone = ?, dancer_mail = ?, dancer_book = ?,class_la_date = NOW(),class_st_date = NOW(),class_date = NOW(),dancer_card = ?,country_code = ?, class_code = ?, class_st_code = ?, class_la_code = ?  WHERE `dancer_code` = ?";
        $this->sql($sql, array("ssssssssssssssssssi", $_POST['dancer_firstname'], $_POST['dancer_lastname'], $_POST['dancer_firstname_en'], $_POST['dancer_lastname_en'], $_POST['dancer_birth'], $_POST['dancer_sex'], $_POST['dancer_index'], $_POST['dancer_street'], $_POST['dancer_house'], $_POST['dancer_flat'], $_POST['dancer_phone'], $_POST['dancer_mail'], $_POST['dancer_book'], $_POST['dancer_card'], $_POST['country_code'], $_POST['class_code'], $_POST['class_st_code'], $_POST['class_la_code'], $_GET['id']));
    }

    function add()
    {
        $sql = "INSERT INTO `sgostu_dancer`(dancer_firstname , dancer_lastname , dancer_firstname_en , dancer_lastname_en , dancer_birth , dancer_sex , dancer_index , dancer_street , dancer_house , dancer_flat , dancer_phone , dancer_mail , dancer_book ,class_la_date,class_st_date,class_date,class_st_code,class_la_code,dancer_card,country_code) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW(),NOW(),?,?,?,?)";
